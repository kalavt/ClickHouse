...
#define KAFKA_RELATED_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(Bool, kafka_aws_msk_iam_auth, false, "Enable AWS MSK IAM authentication (SASL/OAUTHBEARER)", 0) \
    DECLARE(String, kafka_aws_region, "", "AWS region for MSK IAM authentication (e.g., us-east-1). If empty, will be auto-detected from kafka_broker_list.", 0) \
